...
void WriteEvent(LPWSTR pRenderedContent)
{
	...
	hFile = CreateFile(
		L"C:\\Users\\Ting Lin\\Desktop\\Algo SDK Sample\\Algo SDK Sample\\Data.txt",
		FILE_APPEND_DATA,
		0,
		NULL,
		OPEN_ALWAYS,
	...
	printf("Writing %d bytes to TestFile.txt.\n", dwBytesToWrite);
	...
	else
	{
		...
		{
			printf("Wrote %d bytes to TestFile.txt successfully.\n", dwBytesWritten);
		}
	}
...
int size(void) 
{
	...
	//check if glfw first initialize
	if (!glfwInit()) {
		exit(EXIT_FAILURE);
	}
	...
	//creating window
	window = glfwCreateWindow(480, 480, "Size", NULL, NULL);

	//check if window fail
	if (!window) {
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//color --> function make sure the value is less than one
		/*r = fmod(r + 0.01, 1);
		g = fmod(g + 0.02, 1);
		b = fmod(b + 0.03, 1);*/
		...
		//circleSize = (radius + x)*(radius + x)*3.14;
		//buffer = to_string(circleSize);

		circleSize = (radius + x)*(radius + x)*3.14;
		_stprintf(buffer, L"%f\r\n", circleSize);
		WriteEvent(buffer);
	}

	//ballsize = (radius + x)*(radius + x)*3.14;
	//ballSizeV.push_back 
                     ...
                     _In_ int       nCmdShow)
{
	...
	/*HANDLE hFile;
	hFile = CreateFile(
		L"C:\\Users\\Ting Lin\\Desktop\\Algo SDK Sample\\Algo SDK Sample\\TestFile.txt",
